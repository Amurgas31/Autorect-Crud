...
import {
  ...
  CardHeader,
  CardTitle,
} from "@/components/ui/card"; // Importación de componentes de tarjeta personalizados para la interfaz de usuario
import {
  InputOTP,
  InputOTPGroup,
  InputOTPSlot,
} from "@/components/ui/input-otp"; // Importación de componentes personalizados para la entrada de código OTP
...
function OtpVerification() {
  ...
  const handleSubmit = async (event) => {
    ...
    if (otpCode.length !== 6) {
      // Validación básica para asegurarse de que el código OTP tenga exactamente 6 dígitos
  ...
  return (
    <section className="relative min-h-screen overflow-hidden bg-[#1F1F1F] text-white">
      <div
        ...
        style={{
          background:
            "radial-gradient(circle at 20% 18%, rgba(130,39,39,0.35), transparent 38%)",
        }}
      />
      ...
      <div className="relative grid min-h-screen grid-cols-1 lg:grid-cols-2">
        <aside className="hidden lg:flex items-center p-12 lg:p-16">
          <div className="max-w-xl">
            ...
            <h1 className="text-5xl font-bold uppercase tracking-[0.28em] text-white">
              Autorect
            </h1>
            <p className="mt-6 text-2xl leading-7 text-white/50">
              Ingresa el código OTP enviado a tu correo para completar el
              registro.
            </p>
          </div>
        </aside>
          ...
          <Card className="w-full max-w-lg rounded-3xl border-white/10 bg-black/30 p-2 text-white ring-white/5">
            <CardHeader className="px-4 pt-4 sm:px-6 sm:pt-6">
              <CardTitle className="text-3xl font-semibold text-white">
                Código OTP<span className="text-[#822727]">.</span>
              </CardTitle>
              <CardDescription className="mt-2 text-sm text-white/40">
                Revisa tu correo e ingresa los 6 dígitos.
              </CardDescription>
            </CardHeader>
                ...
                <div className="space-y-2">
                  ...
                  >
                    <InputOTPGroup className="rounded-xl border border-white/10 bg-white/4">
                      <InputOTPSlot
                        index={0}
                        className="h-11 w-11 border-white/10 text-white"
                      />
                      <InputOTPSlot
                        index={1}
                        className="h-11 w-11 border-white/10 text-white"
              ...
              <div className="mt-6 flex items-center justify-between text-sm text-white/40">
                <Link
                  ...
                  className={`${buttonVariants({
                    variant: "link",
                    size: "sm",
                  })} h-auto p-0 text-white hover:text-[#822727]`}
                >
                  Volver a registro
                </Link>
                <button
